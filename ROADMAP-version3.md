...
This version adds a new section, **"Cherry-picked from the BoloBee/rebrand
analysis"** below, sourced from a review of an external AI brainstorming
document ("Optimizing Hindi Quest Edtech App") that proposed a much larger
reimagining of the product — a rebrand, a dual-track (ages 3–5 vs. 6+)
architecture, multi-child profiles, a parent gate, microphone-based
speech-echo games, subscriptions, and native app store packaging on a
different tech stack (Vite + Capacitor instead of Expo).

**That fuller reimagining was evaluated and explicitly not adopted** — it's
a speculative brand/business roadmap generated without visibility into this
repo, it assumes a tech stack that duplicates what Expo already gives us
(EAS Build already reaches both app stores; no Capacitor needed), and it
skips past the validation step this roadmap already calls for ("stay
web-only, share the link wider" before investing in anything bigger). It
also proposed three different brand names in one sitting with no
trademark/domain check — not something to act on mid-build.

What *did* survive that review are a handful of low-risk, low-cost UX ideas
that fit the current Expo/React Native codebase directly, need no new
architecture, and are worth testing against real toddler use. Those are
listed below, kept separate from the rest of the roadmap so the source and
reasoning stay traceable.

## Cherry-picked from the BoloBee/rebrand analysis

All four items below are additive to `App.tsx` as it exists today — no new
data model, no rebrand, no accounts/profiles, no new dependencies. Meant to
be tried and evaluated against real usage before going further, not
committed to as a package.

- **Tablet touch-safety lock.** Add the toddler-safe touch CSS (disable
  pinch-zoom, pull-to-refresh, and text-selection callouts;
  `touch-action: none` at the root) and lock orientation to landscape.
  Toddlers rest palms on screens and pinch/scroll by accident — this is a
  pure friction removal, not a design change, and costs a few lines in the
...
- **Warmer palette pass.** The analysis (working only from a live
  ...
  tones instead. Worth a sanity check against the actual design rationale
  in `DESIGN.md` before changing anything — if the critique holds up,
  it's a token/color-value change, not a structural one.
- **Less English text in onboarding/UI chrome.** Where a toggle or label
  currently requires English reading literacy (e.g. language/pronunciation
  toggles), lean further on icons, color, and voice prompts so a
  non-reading 3-5 year old can navigate unassisted. Complements work
  already done on audio pacing (see `SPEECH_RATE` history in `STATUS.md`).
- **Non-punitive wrong-answer feedback, audited.** Confirm the current
  wrong-answer state (visual/audio) reads as gentle redirection rather than
  a buzzer/failure state, in the same spirit as the existing "Find the
  Opposite" and Match-and-Listen flows. Likely already mostly true given
  the audio-clipping fixes already made — this is a verification pass, not
  necessarily new code.

Sequencing: these sit behind Tamil verification in priority (still the
single biggest open item — see below), and are meant to be tried in the
existing web build with a few families before any larger direction (new
interaction templates, dual-track, rebrand) is considered.
...
- **Stay web-only, share the link wider.** Zero additional cost or
  ...
  real before spending on anything else — and the right gate to pass
  *before* any of the bigger, business-shaped ideas below (profiles,
  subscriptions, rebrand, native distribution) are worth revisiting.
